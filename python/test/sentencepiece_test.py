...
class TestSentencepieceProcessor(unittest.TestCase):
  ...
  def test_load(self):
    self.assertEqual(32000, self.sp_.GetPieceSize())
  ...
  def test_train(self):
    spm.SentencePieceTrainer.Train(
        '--input=' + os.path.join(data_dir, 'wagahaiwa_nekodearu.txt') +
        ' --model_prefix=m --vocab_size=32000')
    sp = spm.SentencePieceProcessor()
    sp.Load('m.model')
    with codecs.open(os.path.join(data_dir, 'wagahaiwa_nekodearu.txt'), 'r', encoding='utf-8') as file:
      for line in file:
  ...
  def test_load_snake(self):
    self.assertEqual(32000, self.sp_.get_piece_size())
  ...
  def test_train_snake(self):
    ...
    with codecs.open(os.path.join(data_dir, 'wagahaiwa_nekodearu.txt'), 'r', encoding='utf-8') as file:
      for line in file:
        a=sp.decode_pieces(sp.encode_as_pieces(line))
        b=sp.decode_ids(sp.encode_as_ids(line))
...
def test_train():
    spm.SentencePieceTrainer.Train(
        '--input=' + os.path.join(data_dir, 'wagahaiwa_nekodearu.txt') +
        ' --model_prefix=m --vocab_size=3000 --hard_vocab_limit=false')
    sp = spm.SentencePieceProcessor()
    sp.Load('m.model')
    i=0
    with codecs.open(os.path.join(data_dir, 'wagahaiwa_nekodearu.txt'), 'r', encoding='utf-8') as file:
      for line in file:
        a=sp.DecodePieces(sp.EncodeAsPieces(line))
        b=sp.DecodeIds(sp.EncodeAsIds(line))
        i=i+1
        # print(len(b),i,b)
